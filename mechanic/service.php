  ...
  <body>
    ...
    <div class="row no-gutters mt-5">
        ...
        <div class="col-md-10 p-5 pt-2">
            <h3><i class="fas fa-cog" style="padding-right : 20px;"></i>SERVICE</h3>
            ...
            <hr>
            <a href="laporservice.php" class="btn btn-primary mb-2">LAPOR SERVICE</a>
            <div class="tabel">
            <table class="table">
            <thead class="thead-dark">
                 ...
                 <tbody>
                  <?php
                   ...
                   $data = mysqli_query($conn,"SELECT * FROM service_form INNER JOIN login ON service_form.id_login = login.id_login WHERE login.id_login = '".$_SESSION['id_login']."'");
                   ...
                   if(isset($_GET['cari'])){
                    $cari = $_GET['cari'];
                    $sql = "SELECT service_form.id_service, login.id_login, login.nama, service_form.id_login, service_form.nama, service_form.tgl_service, service_form.jenis_service, service_form.merk_service, service_form.model_service, service_form.sn_service, service_form.hour_meter, service_form.jml_oli_msn, service_form.jml_oli_hyd, service_form.jml_solar, service_form.flt_oli, service_form.flt_solar, service_form.flt_udara, service_form.status_service, login.level FROM service_form INNER JOIN login ON service_form.id_login = login.id_login WHERE login.id_login = '".$_SESSION['id_login']."' AND  service_form.model_service like '%".$cari."%'";				
                }else{

                  $sql = "SELECT service_form.id_service, login.id_login, login.nama, service_form.id_login, service_form.nama, service_form.tgl_service, service_form.jenis_service, service_form.merk_service, service_form.model_service, service_form.sn_service, service_form.hour_meter, service_form.jml_oli_msn, service_form.jml_oli_hyd, service_form.jml_solar, service_form.flt_oli, service_form.flt_solar, service_form.flt_udara, service_form.status_service, service_form.hour_meter,login.level FROM service_form INNER JOIN login ON service_form.id_login = login.id_login WHERE login.id_login = '".$_SESSION['id_login']."' limit $halaman_awal, $batas";
                }
                  $result =$conn-> query($sql);
                      if($result->num_rows > 0){
                        ...
                        while($row = $result->fetch_assoc()){
                          ...
                          echo '<td>'.$row['merk_service'].'</td>';
                          echo '<td>'.$row['model_service'].'</td>';
                          echo '<td>'.$row['sn_service'].'</td>';
                          echo '<td>'.$row['hour_meter'].'</td>';?>
                          
                          <td><a href="http:detailservice.php?id_service=<?php echo $row['id_service'];?>"><button class="btn btn-primary">Detail</button></a></td>
